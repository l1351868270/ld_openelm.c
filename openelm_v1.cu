...
nvcc -o openelm_v1_cu openelm_v1.cu -lm
./openelm_v1_cu
seconds:4.845000s tokens:256 achieved tok/s: 52.837977

nvcc -o openelm_v1_cu -O3 openelm_v1.cu -lm
./openelm_v1_cu
...
void group_attention_forward(float* output, float *q, float *key_cache, float *value_cache, float *att,
    ...
    for (int lk = lid; lk < pos + 1; lk += WARP_THREADS) { 
    ...
    #pragma unroll
    for (int mask = 32 / 2; mask > 0; mask /= 2) {
        float shfl_max = __shfl_xor_sync(uint32_t(-1), max_val, mask);
        if (shfl_max > max_val) {
            max_val = shfl_max;
        }
        __syncwarp();
