...
nvcc -o openelm_v2_cu openelm_v2.cu -lm
./openelm_v2_cu
seconds:4.845000s tokens:256 achieved tok/s: 52.837977

nvcc -o openelm_v2_cu -O3 openelm_v2.cu -lm
./openelm_v2_cu
seconds:4.838000s tokens:256 achieved tok/s: 52.914427
...
void qkv_forward(float* input, float *q, float *qkv_proj, float *q_norm, float *k_norm, 
    ...
    int out = bidy * WARPGROUP_THREADS + tid;
...
void key_rope_forward(float *key_cache, float *k_norm, int rope_freq_constant_, 
    ...
    for (int hd = tid; hd < head_dim; hd += WARPGROUP_THREADS) {
...
void group_flash_attention_forward(float* output, float *q, float *key_cache, float *value_cache, float *att,
    ...
    *d = 1.0f;
    ...
    for (int lk = 1; lk < pos + 1; lk++) {
        ...
        d_i = d_i1 * expf(m_i1 - m_i) + expf(score - m_i);
        ...
        for (int lv = tid; lv < head_dim; lv += kNThreads){
            ...
            int offset_v = b * num_transformer_layers * max_seq_len * max_kv_heads * head_dim 
                         ...
                         + lk * max_kv_heads * head_dim
                         + (h / num_groups) * head_dim;
            o_i = o_i1 * (d_i1 * expf(m_i1 - m_i) / d_i) + expf(score - m_i) / d_i * value_cache[offset_v + lv];
            o[lv] = o_i;
...
float* openelm_forward(Context *ctx, OpenELM* openelm, int *token, int batch, int pos) {
    ...
    get_content_row<<<dim3(batch, model_dim/WARPGROUP_THREADS), WARPGROUP_THREADS>>>(x, w->token_embeddings, token, batch, model_dim);
    
    // for(int l = 0; l < 1; l++) {
    for(int l = 0; l < p->num_transformer_layers; l++) {
        // attn_norm
        s->flops += batch * model_dim * (2 + 2 * model_dim / WARP_THREADS + 5 + 3 + 12);
        s->flops_sfu += batch * model_dim * (1);
        rmsnorm_forward<<<dim3(batch, model_dim / WARPGROUP_THREADS), WARPGROUP_THREADS>>>(s->xb, s->x, w->attn_norm + l*model_dim, batch, model_dim);
        ...
        //
        s->flops += batch * out_features * (4 + out_features + 13 + model_dim * 7 + 2);
        qkv_forward<<<dim3(batch, out_features / WARPGROUP_THREADS), WARPGROUP_THREADS>>>(s->xb, s->q, w->qkv_proj, w->q_norm, w->k_norm, 
        ...
        // rope
        s->flops += batch * q_heads * WARPGROUP_THREADS * (5 + head_dim/WARP_THREADS * 2 + 5 + 3 + (head_dim + WARPGROUP_THREADS - 1)/WARPGROUP_THREADS * 6);
        query_rope_forward<<<dim3(batch, q_heads), WARPGROUP_THREADS>>>(s->q, w->q_norm, p->rope_freq_constant, batch, q_heads, head_dim, l, pos);
        ...
        // group attention
        group_flash_attention_forward<<<dim3(batch, q_heads),kNThreads>>>(s->xb, s->q, s->key_cache, s->value_cache, s->att,
...
void generate(Context *ctx, OpenELM *openelm, Prompt *prompt, int steps) {
    ...
    // while (pos < steps) {
        ...
        argmax_forward<<<dim3(s->batch, openelm->config.vocab_size / WARPGROUP_THREADS), WARPGROUP_THREADS>>>(s->next, s->logits, s->batch, openelm->config.vocab_size);
    ...
    fprintf(stderr, "cuda kernel seconds:%fs tokens:%d achieved tok/s: %f\n", elapsedTime, pos, (pos) / (double)(end-start)*1000);
}
...
int start() {
    ...
    Context ctx;
    generate(&ctx, &model, &prompt, model.state.max_seq_len);
    printf("hello openelm\n");
    return 0;
}
